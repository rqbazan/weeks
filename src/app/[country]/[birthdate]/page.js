...
function Cell({ borderful, disabled, className, styles, children }) {
  return (
    <span
      style={styles}
      className={clsx(
        "flex w-4 h-4 flex-shrink-0 text-gray-300 font-mono",
...
function Matrix({ userWeeks, lifeExpectancyWeeks }) {
  return LIFE_IN_YEARS.map((_, x) => {
          ...
          return (
            <Cell
              ...
              className={clsx({
                ...
                "mr-4": (y + 1) % 4 === 0 && y !== WEEKS_IN_ONE_YEAR.length - 1,
              })}
              disabled={c2 === 0}
              styles={{
                "--c1": `${c1 * 100}%`,
...
export default async function Page({ params }) {
  const { lifeExpectancyYears, userWeeks, lifeExpectancyWeeks } = calculate(
    await params
  );
  ...
  return (
    <div className="md:max-w-7xl md:ml-auto md:mr-auto">
      ...
      <section className="flex flex-col md:flex-row items-center md:justify-center px-6 md:px-0 space-y-2 md:space-x-4 md:space-y-0 font-mono mb-4">
      ...
      <div className="flex p-1 pt-0 pr-2 md:p-4 md:pt-0">
        ...
        <div className="flex flex-col flex-grow min-w-0">
          <div className="flex items-end pb-2 h-8 sticky bg-gray-700 top-0 z-10">
            <header
              className="syncscroll flex w-full overflow-x-auto overflow-y-hidden justify-between $hidden-scrollbar"
              name="scrollable-matrix"
            >
          ...
          <div
            className="syncscroll flex flex-col w-full overflow-x-auto"
            name="scrollable-matrix"
          >
            <Matrix
              userWeeks={userWeeks}
              lifeExpectancyWeeks={lifeExpectancyWeeks}
            />
          </div>
        </div>
      </div>
